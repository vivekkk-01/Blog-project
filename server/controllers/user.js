  ...
  try {
    ...
    if (
      followedUser.followers.find(
        (follower) => follower.toString() === loggedInUserId.toString()
      )
    ) {
...
exports.updateUnfollow = async (req, res) => {
  try {
    ...
    if (
      loggedInUser.followings.find(
        (follow) => follow.toString() === unfollowId.toString()
      )
    ) {
...
exports.forgotPasswordToken = async (req, res) => {
  try {
    ...
    await sgMail.send(msg);
    return res.json(
      `A verification message is sent successfully to ${email}. Reset now within 10 minutes, ${resetUrl}.`
    );
  } catch (error) {
    return res.status(500).json("Something went wrong, please try again!");
  }
